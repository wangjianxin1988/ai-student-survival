...
const CATEGORIES: { value: CommunityCategory; label: string; description: string }[] = [
  { value: 'tools', label: 'AI工具', description: '分享AI工具的使用体验、评测' },
  { value: 'payment', label: '支付指南', description: '介绍支付方式、换汇技巧' },
  { value: 'policy', label: '大学政策', description: '分享大学AI使用政策' },
  { value: 'prompt', label: 'Prompt', description: '分享有用的Prompt模板' },
  { value: 'survival', label: '妙妙贴', description: '分享留学生活技巧、防坑指南' },
  { value: 'discussion', label: '讨论', description: '自由讨论话题' },
  { value: 'qa', label: '问答', description: '提问和解答' },
];

// AI工具表单字段
function ToolsForm({ data, onChange }: {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">定价模式 *</label>
          <select
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => onChange('priceMonthly', parseFloat(e.target.value))}
              placeholder="0"
              className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => onChange('priceYearly', parseFloat(e.target.value))}
              placeholder="0"
              className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">官网URL *</label>
        <input
          type="url"
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">工具描述 *</label>
        <textarea
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">主要功能（每行一个）</label>
        <textarea
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">同类替代工具（每行一个）</label>
        <textarea
          value={((data.alternatives as string[]) || []).join('\n')}
          onChange={(e) => onChange('alternatives', e.target.value.split('\n').filter(a => a.trim()))}
          placeholder="替代工具1&#10;替代工具2"
          rows={3}
          className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
        />
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">使用技巧（每行一条）</label>
        <textarea
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">使用教程（每行一个步骤，格式：标题|描述）</label>
        <textarea
          value={((data.howToUse as Array<{step: number; title: string; description: string}>) || []).map((h, i) => `${i + 1}. ${h.title}|${h.description}`).join('\n')}
          onChange={(e) => {
            const lines = e.target.value.split('\n').filter(l => l.trim());
            const howToUse = lines.map((line, i) => {
              const [title, ...descParts] = line.split('|');
              return { step: i + 1, title: title.replace(/^\d+\.\s*/, '').trim(), description: descParts.join('|').trim() };
            });
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">使用场景（每行一个，格式：标题|描述）</label>
        <textarea
          value={((data.useCases as Array<{title: string; description: string}>) || []).map(u => `${u.title}|${u.description}`).join('\n')}
          onChange={(e) => {
            const lines = e.target.value.split('\n').filter(l => l.trim());
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">可靠性</label>
          <select
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">适用的AI工具（每行一个）</label>
        <textarea
          value={((data.toolIds as string[]) || []).join('\n')}
          onChange={(e) => onChange('toolIds', e.target.value.split('\n').filter(t => t.trim()))}
          placeholder="ChatGPT&#10;Claude&#10;Midjourney"
          rows={3}
          className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
        />
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">详细教程 *</label>
        <textarea
...
function PolicyForm({ data, onChange }: {
  ...
  const handleUniversityNameChange = (name: string) => {
    ...
    if (!data.universitySlug || (data.universityName && data.universityName !== name)) {
      const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">总体政策 *</label>
        <select
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">教学场景政策 *</label>
        <textarea
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">作业政策 *</label>
        <textarea
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">考试政策 *</label>
        <textarea
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">允许使用的AI工具（逗号分隔）</label>
        <input
          type="text"
          value={((data.allowedTools as string[]) || []).join(', ')}
          onChange={(e) => onChange('allowedTools', e.target.value.split(',').map(t => t.trim()).filter(t => t))}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">限制使用的AI工具（逗号分隔）</label>
        <input
          type="text"
          value={((data.restrictedTools as string[]) || []).join(', ')}
          onChange={(e) => onChange('restrictedTools', e.target.value.split(',').map(t => t.trim()).filter(t => t))}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">禁止使用的AI工具（逗号分隔）</label>
        <input
          type="text"
          value={((data.prohibitedTools as string[]) || []).join(', ')}
          onChange={(e) => onChange('prohibitedTools', e.target.value.split(',').map(t => t.trim()).filter(t => t))}
          placeholder="明确的禁止工具"
          className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">小组项目政策</label>
        <textarea
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">编程场景政策</label>
        <textarea
...
// Prompt表单字段
function PromptForm({ data, onChange }: {
  data: Record<string, unknown>;
  onChange: (field: string, value: unknown) => void;
}) {
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Prompt分类 *</label>
        <select
...
// 妙妙贴表单字段
function SurvivalForm({ data, onChange }: {
  data: Record<string, unknown>;
  onChange: (field: string, value: unknown) => void;
}) {
  return (
...
// 讨论/问答表单字段
function DiscussionForm({ data, onChange, isQA }: {
  data: Record<string, unknown>;
  onChange: (field: string, value: unknown) => void;
  isQA: boolean;
}) {
...
export function PostEditor({ initialData, onSubmit, onCancel }: PostEditorProps) {
  ...
  const [category, setCategory] = useState<CommunityCategory>(initialData?.category || 'discussion');
  const [tagsInput, setTagsInput] = useState(initialData?.tags?.join(', ') || '');
  const [meta, setMeta] = useState<Record<string, unknown>>(initialData?.meta || {});
  ...
  const validateForm = (): string | null => {
    ...
    switch (category) {
      ...
      case 'payment':
        ...
        if (!Array.isArray(meta.toolIds) || meta.toolIds.length === 0) return '请选择关联工具';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (honeypot) {
      ...
      setLoading(false);
      onSubmit?.({ title: title.trim(), content: content.trim(), category, tags: [], meta });
      return;
    }
    ...
    if (meta.content && ['payment', 'policy', 'prompt', 'survival'].includes(category)) {
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <div className="text-right text-xs text-gray-400 mt-1">{title.length}/255</div>
        </div>
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            ...
            {(category === 'discussion' || category === 'qa') && (category === 'qa' ? '问题详情' : '话题详情')}
          </label>
          {category === 'tools' && <ToolsForm data={meta} onChange={handleMetaChange} />}
          {category === 'payment' && <PaymentForm data={meta} onChange={handleMetaChange} />}
          {category === 'policy' && <PolicyForm data={meta} onChange={handleMetaChange} />}
          {category === 'prompt' && <PromptForm data={meta} onChange={handleMetaChange} />}
          {category === 'survival' && <SurvivalForm data={meta} onChange={handleMetaChange} />}
          {(category === 'discussion' || category === 'qa') && <DiscussionForm data={meta} onChange={handleMetaChange} isQA={category === 'qa'} />}
        </div>

        {/* Only show extra content field for simple posts */}
        {category === 'discussion' && (
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2">补充内容</label>
            <textarea
              value={content}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm">
                ...
                <ul className="text-blue-600 space-y-1">
                  <li>• 热度达标后自动推送到 {CATEGORY_LABELS[category]?.zh || '该分类'} 板块</li>
