...
const translations = {
  zh: {
    ...
    passwordWeak: '密码强度：弱 (建议使用8位以上，包含大小写、数字和特殊字符)',
    passwordMedium: '密码强度：中 (建议添加特殊字符)',
...
export default function RegisterForm({ locale = 'zh', onSwitchToLogin, returnTo }: RegisterFormProps) {
  ...
  const [isDemoMode] = useState(true); // Always demo mode without Supabase
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('RegisterForm: calling demoAuthApi.signUp');
  ...
  const handleAlipaySignIn = async () => {
    const result = await demoAuthApi.signInWithOAuth('alipay');
    if (result.error) {
      setError(result.error);
    }
  };
    ...
    <div className="w-full max-w-md mx-auto">
      ...
      {isDemoMode && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-700 text-sm">
          ...
          <div className="text-yellow-600">{t.demoModeHint}</div>
        </div>
      )}
          ...
          {password && (
            <div className="mt-2">
              <div className="flex gap-1 mb-1">
                <div className={`h-1 flex-1 rounded ${passwordStrength.level === 'weak' ? 'bg-red-500' : passwordStrength.level === 'medium' ? 'bg-yellow-500' : 'bg-green-500'}`} />
                <div className={`h-1 flex-1 rounded ${passwordStrength.level === 'medium' || passwordStrength.level === 'strong' ? (passwordStrength.level === 'medium' ? 'bg-yellow-500' : 'bg-green-500') : 'bg-gray-200'}`} />
                <div className={`h-1 flex-1 rounded ${passwordStrength.level === 'strong' ? 'bg-green-500' : 'bg-gray-200'}`} />
              </div>
              <p className={`text-xs ${passwordStrength.level === 'weak' ? 'text-red-500' : passwordStrength.level === 'medium' ? 'text-yellow-600' : 'text-green-600'}`}>
                {passwordStrength.level === 'weak' ? t.passwordWeak : passwordStrength.level === 'medium' ? t.passwordMedium : t.passwordStrong}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="mt-6">
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          {/* Alipay */}
          <button
            onClick={handleAlipaySignIn}
            className="flex items-center justify-center gap-2 py-2.5 px-4 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="#1677FF">
              <path d="M230.4 76.8c-3.2 0-6.4 1.6-8 4L204.8 112H268.8l-12.8-25.6c-1.6-3.2-4-4.8-7.2-4.8-4 0-8 3.2-8 8v44.8c0 4.4 3.6 8 8 8s8-3.6 8-8V125.6l19.2 38.4c1.6 3.2 4 4.8 7.2 4.8 4 0 8-3.2 8-8v-70.4c0-4.4-3.6-8-8-8zM24 44.8c-4.4 0-8 3.6-8 8s3.6 8 8 8 8-3.6 8-8-3.6-8-8-8zm4 36.8l-12.8 24c-1.6 3.2-1.6 6.4 0 9.6 1.6 3.2 4 4.8 7.2 4.8H48V96H28c-4.4 0-8 3.6-8 8 0 4.4 3.6 8 8 8h4.8L48 129.6c1.6-3.2 4-4.8 7.2-4.8h46.4V108.8H55.2c-4.4 0-8 3.6-8 8 0 4.4 3.6 8 8 8h38.4L108.8 140c-1.6 3.2-1.6 6.4 0 9.6 1.6 3.2 4 4.8 7.2 4.8h48v-16H120c-4.4 0-8 3.6-8 8 0 4.4 3.6 8 8 8h44c3.2 0 6.4-1.6 8-4l16-32c1.6-3.2 1.6-6.4 0-9.6-1.6-3.2-4-4.8-7.2-4.8H172c-4.4 0-8 3.6-8 8v16c0 4.4 3.6 8 8 8h8v-16c0-4.4-3.6-8-8-8z"/>
            </svg>
            <span className="text-sm font-medium">{t.alipaySignIn}</span>
          </button>
        </div>
      </div>
      ...
      <p className="mt-6 text-center text-sm text-gray-600">
        {t.hasAccount}{' '}
        <button
          onClick={onSwitchToLogin ?? (() => window.location.href = '/auth/login')}
          className="text-primary-600 hover:text-primary-700 font-medium"
        >
          {t.login}
        </button>
