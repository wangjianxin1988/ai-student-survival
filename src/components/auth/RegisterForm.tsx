...
export default function RegisterForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success && result.verificationRequired) {
        // New user created — show OTP verification form and send code
        setOtpSent(true);
        if (result.userId) {
        ...
        // Fully confirmed (demo mode, etc.) — redirect
        redirectAfterRegister();
      } else if (result.verificationRequired) {
        // Edge case: verificationRequired but not success
        setOtpSent(true);
        if (result.userId) {
          setSignupUserId(result.userId);
        }
  ...
  // Show "check your email" prompt after signup
  if (otpSent) {
    return (
      <div className="w-full max-w-md mx-auto">
        ...
        <div className="text-center py-4">
          ...
          <h3 className="text-lg font-bold mb-2">
            {locale === "zh" ? "请查收验证邮件" : "Check Your Email"}
          </h3>
          ...
          <p className="text-gray-500 text-sm mb-6">
            {locale === "zh"
              ? "请点击邮件中的链接完成验证。验证完成后即可登录。邮件可能在垃圾邮件文件夹中。"
              : "Click the link in the email to verify your account. Check your spam folder if you don't see it."}
          </p>

          <a
            href={locale === "zh" ? "/auth/login" : "/en/auth/login"}
            className="inline-block px-6 py-2.5 bg-primary-500 hover:bg-primary-600 text-white font-medium rounded-lg transition-colors mb-4"
          >
          ...
          <div className="mt-4">
            ...
            >
              {resending
                ...
                : (resendSent
                  ? (locale === "zh" ? "已重新发送 ✓" : "Resent ✓")
                  : (locale === "zh" ? "没有收到？重新发送" : "Didn't receive? Resend"))}
            </button>
          </div>
        </div>
      </div>
