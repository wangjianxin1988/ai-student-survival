...
export default function AddMarkerForm({ initialCoordinates, onSubmit, onCancel }: AddMarkerFormProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('coordinates-updated', handleCoordinatesUpdate as EventListener);
    return () => {
      window.removeEventListener('coordinates-updated', handleCoordinatesUpdate as EventListener);
    };
  }, []);
  ...
  const handleUniversityChange = (newUniversityId: string) => {
    ...
    window.dispatchEvent(new CustomEvent('university-changed', {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const event = new CustomEvent('marker-form-submit', { detail: markerData, bubbles: true });
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        >
          {universities.map(uni => (
            <option key={uni.id} value={uni.id}>{uni.nameZh}</option>
          ))}
        </select>
      </div>
      <div>
      ...
      <div>
        ...
        <select
          ...
          onChange={e => setFormData(prev => ({ ...prev, category: e.target.value as MapMarkerCategory }))}
          className="w-full px-3 py-2 border border-gray-300 rounded-lg"
        >
      ...
      <div className="flex gap-3 pt-4">
        <button type="button" onClick={onCancel || (() => window.history.back())} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg">取消</button>
        <button type="submit" className="flex-1 px-4 py-2 bg-blue-500 text-white rounded-lg">提交</button>
      </div>
    </form>
  );
}
