...
function getQuestionsByTarget(targetType: string, targetId: string): QAQuestion[] {
...
export default function QASystem({ targetType, targetId, targetName, locale = 'zh' }: QASystemProps) {
  ...
  const handleLikeQuestion = (question: QAQuestion) => {
    ...
    if (question.likedBy.includes(user.id)) {
      allQuestions[qIndex].likedBy = allQuestions[qIndex].likedBy.filter(id => id !== user.id);
  ...
  const handleLikeAnswer = (answer: QAAnswer) => {
    ...
    if (answer.likedBy.includes(user.id)) {
      allAnswers[aIndex].likedBy = allAnswers[aIndex].likedBy.filter(id => id !== user.id);
  ...
  const answers = selectedQuestion ? getAnswersByQuestion(selectedQuestion.id) : [];
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            />
            <div>
              <div className="font-medium text-gray-900">{selectedQuestion.userName}</div>
              <div className="text-sm text-gray-500">
                {new Date(selectedQuestion.createdAt).toLocaleDateString(locale === 'zh' ? 'zh-CN' : 'en-US')}
              </div>
            </div>
          ...
          <div className="flex items-center gap-4 text-sm text-gray-500">
            ...
            >
              <svg className="w-4 h-4" fill={user && selectedQuestion.likedBy.includes(user.id) ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
              </svg>
              {selectedQuestion.likes}
            </button>
            <span>{selectedQuestion.views} {t.views}</span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {answers.map((answer) => (
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="text-sm">
                  ...
                  <span className="text-gray-500 ml-2">
                    {new Date(answer.createdAt).toLocaleDateString(locale === 'zh' ? 'zh-CN' : 'en-US')}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  <svg className="w-4 h-4" fill={user && answer.likedBy.includes(user.id) ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                  </svg>
                  {answer.likes} {t.helpful}
                </button>

                {!answer.isAccepted && selectedQuestion.status !== 'closed' && user?.id === selectedQuestion.userId && (
                  <button
                    onClick={() => handleAcceptAnswer(selectedQuestion, answer)}
          ...
          {selectedQuestion.status !== 'closed' && (
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <h4 className="font-semibold text-gray-900 mb-3">{t.writeAnswer}</h4>
              <textarea
                value={answerContent}
              ...
              >
                {submitting ? (locale === 'zh' ? '提交中...' : 'Submitting...') : t.submitAnswer}
              </button>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="space-y-3">
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {submitting ? (locale === 'zh' ? '提交中...' : 'Submitting...') : t.submitQuestion}
              </button>
            </div>
          </div>
        </div>
              ...
              <div className="flex items-start gap-3">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-2 text-xs text-gray-400">
                    ...
                    <span>{new Date(question.createdAt).toLocaleDateString(locale === 'zh' ? 'zh-CN' : 'en-US')}</span>
                    <span>{getAnswersByQuestion(question.id).length} {t.answers}</span>
                  </div>
                </div>
